...
final class UiUtils {
    ...
    public static func updateCardBrandImage(_ imageView: UIImageView, with cardBrand: CardBrand?) {
            ...
            let url = URL(string: urlAsString)
        {
            if let cachedImage = cardBrandsImagesCache[urlAsString] {
                setImageSourceAndLabel(imageView, source: UIImage(data: cachedImage.uiImage)!, label: cachedImage.label)
            } else {
            ...
            DispatchQueue.main.async {
                setImageSourceAndLabel(imageView, source: unknownBrandImage, label: "unknown_card_brand")
    ...
    private static func setImageSourceAndLabel(_ imageView: UIImageView, source: UIImage, label: String) {
        DispatchQueue.main.async {
