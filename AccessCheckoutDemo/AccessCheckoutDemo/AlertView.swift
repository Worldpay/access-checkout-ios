...
class AlertView {
    ...
    static func display(using uiViewController: UIViewController, title: String?, message: String?, closeHandler: @escaping () -> Void) {
    ...
    private func display(using uiViewController: UIViewController) {
        ...
        alertController.addAction(UIAlertAction(title: self.closeButtonText, style: .default, handler: { action in
            self.closeHandler?()
