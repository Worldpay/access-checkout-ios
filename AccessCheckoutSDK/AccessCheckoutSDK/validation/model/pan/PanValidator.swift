class PanValidator {
    ...
    func canValidate(_ pan: String) -> Bool {
        ...
        return validationRule.textIsMatched(strippedPan)
            && validationRule.textIsShorterOrAsLongAsMaxLength(strippedPan)
    }
    ...
    private func isAcceptedBrand(_ config: CardBrandsConfiguration, _ cardBrand: CardBrandModel?)
        -> Bool
    {
        return config.acceptedCardBrands.isEmpty
            || config.acceptedCardBrands.contains(where: {
                $0.lowercased() == cardBrand!.name.lowercased()
            })
    }
}
