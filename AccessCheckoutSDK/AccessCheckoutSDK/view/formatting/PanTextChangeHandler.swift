...
class PanTextChangeHandler {
    ...
    func change(originalText: String, textChange: String, usingSelection selection: NSRange)
        -> String
    {
        ...
        let resultingText = stripWhiteSpaces(
            from: originalText.replacingCharacters(in: selection, with: textChange)
        )
        let validationResult = panValidator.validate(pan: resultingText)
        let maxLengthAllowed =
            (validationResult.cardBrand?.panValidationRule.validLengths.max()
            ?? ValidationRulesDefaults.instance().pan.validLengths.max())!

        let textCutToMaxLength =
            resultingText.count <= maxLengthAllowed
            ? resultingText : left(nCharacters: maxLengthAllowed, of: resultingText)
