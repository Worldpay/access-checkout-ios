...
class ExpiryDateViewPresenter: NSObject, Presenter {
    ...
    func textFieldEditingChanged(_ textField: UITextField) {
        ...
        if !text.isEmpty {
            let isNotDeletingSeparator = !attemptingToDeleteSeparator(
                textBefore: textBeforeEditingChanged,
                textAfter: text
            )
    ...
    private func attemptingToDeleteSeparator(textBefore: String, textAfter: String) -> Bool {
        if textBefore.hasSuffix(expiryDateFormatter.separator),
            !textAfter.hasSuffix(expiryDateFormatter.separator)
        {
...
extension ExpiryDateViewPresenter: UITextFieldDelegate {
    ...
    public func textField(
        ...
        let resultingText = textChangeHandler.change(
            originalText: textField.text,
            textChange: string,
            usingSelection: range
        )
