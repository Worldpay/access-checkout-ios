...
@testable import AccessCheckoutSDK
 ...
 class MockCvcValidationStateHandler: CvcValidationStateHandler, Cuckoo.ProtocolMock {
    ...
    return cuckoo_manager.call("handleCvcValidation(isValid: Bool)",
            parameters: (isValid),
    ...
    return cuckoo_manager.call("notifyMerchantOfCvcValidationState()",
            parameters: (),
            escapingParameters: (),
            superclassCall:
	 ...
	 struct __StubbingProxy_CvcValidationStateHandler: Cuckoo.StubbingProxy {
	    ...
	    func handleCvcValidation<M1: Cuckoo.Matchable>(isValid: M1) -> Cuckoo.ProtocolStubNoReturnFunction<(Bool)> where M1.MatchedType == Bool {
	        ...
	        return .init(stub: cuckoo_manager.createStub(for: MockCvcValidationStateHandler.self, method: "handleCvcValidation(isValid: Bool)", parameterMatchers: matchers))
	    }
	    
	    func notifyMerchantOfCvcValidationState() -> Cuckoo.ProtocolStubNoReturnFunction<()> {
	        let matchers: [Cuckoo.ParameterMatcher<Void>] = []
	        return .init(stub: cuckoo_manager.createStub(for: MockCvcValidationStateHandler.self, method: "notifyMerchantOfCvcValidationState()", parameterMatchers: matchers))
	    }
	 ...
	 struct __VerificationProxy_CvcValidationStateHandler: Cuckoo.VerificationProxy {
	    ...
	    func handleCvcValidation<M1: Cuckoo.Matchable>(isValid: M1) -> Cuckoo.__DoNotUse<(Bool), Void> where M1.MatchedType == Bool {
	        let matchers: [Cuckoo.ParameterMatcher<(Bool)>] = [wrap(matchable: isValid) { $0 }]
	        return cuckoo_manager.verify("handleCvcValidation(isValid: Bool)", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    
	    @discardableResult
	    func notifyMerchantOfCvcValidationState() -> Cuckoo.__DoNotUse<(), Void> {
	        let matchers: [Cuckoo.ParameterMatcher<Void>] = []
	        return cuckoo_manager.verify("notifyMerchantOfCvcValidationState()", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
