...
class MockApiResponseLinkLookup: ApiResponseLinkLookup, Cuckoo.ClassMock {
    ...
    struct __StubbingProxy_ApiResponseLinkLookup: Cuckoo.StubbingProxy {
        ...
        func lookup<M1: Cuckoo.Matchable, M2: Cuckoo.Matchable>(link: M1, in apiResponse: M2)
            -> Cuckoo.ClassStubFunction<(String, ApiResponse), String?>
        where M1.MatchedType == String, M2.MatchedType == ApiResponse {
            let matchers: [Cuckoo.ParameterMatcher<(String, ApiResponse)>] = [
                wrap(matchable: link) { $0.0 }, wrap(matchable: apiResponse) { $0.1 },
            ]
            return .init(
                stub: cuckoo_manager.createStub(
                    for: MockApiResponseLinkLookup.self,
                    method:
                        """
                        lookup(link: String, in: ApiResponse) -> String?
    ...
    struct __VerificationProxy_ApiResponseLinkLookup: Cuckoo.VerificationProxy {
        ...
        @discardableResult
        func lookup<M1: Cuckoo.Matchable, M2: Cuckoo.Matchable>(link: M1, in apiResponse: M2)
            -> Cuckoo.__DoNotUse<(String, ApiResponse), String?>
        where M1.MatchedType == String, M2.MatchedType == ApiResponse {
            let matchers: [Cuckoo.ParameterMatcher<(String, ApiResponse)>] = [
                wrap(matchable: link) { $0.0 }, wrap(matchable: apiResponse) { $0.1 },
            ]
