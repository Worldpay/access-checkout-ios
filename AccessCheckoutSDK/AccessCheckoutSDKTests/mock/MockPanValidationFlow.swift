...
import Foundation
 ...
 class MockPanValidationFlow: PanValidationFlow, Cuckoo.ClassMock {
    ...
    """,
     ...
     override func getFirstCardBrand() -> CardBrandModel? {
        
    return cuckoo_manager.call(
    """
    getFirstCardBrand() -> CardBrandModel?
    """,
            parameters: (),
            escapingParameters: (),
            superclassCall:
                
                super.getFirstCardBrand()
                ,
            defaultCall: __defaultImplStub!.getFirstCardBrand())
    ...
    """, parameterMatchers: matchers))
        ...
        func handleCobrandedCards<M1: Cuckoo.Matchable>(pan: M1) -> Cuckoo.ClassStubNoReturnFunction<(String)> where M1.MatchedType == String {
            let matchers: [Cuckoo.ParameterMatcher<(String)>] = [wrap(matchable: pan) { $0 }]
            return .init(stub: cuckoo_manager.createStub(for: MockPanValidationFlow.self, method:
    """
    handleCobrandedCards(pan: String)
    ...
    """, parameterMatchers: matchers))
        ...
        func getFirstCardBrand() -> Cuckoo.ClassStubFunction<(), CardBrandModel?> {
            let matchers: [Cuckoo.ParameterMatcher<Void>] = []
            return .init(stub: cuckoo_manager.createStub(for: MockPanValidationFlow.self, method:
    """
    getFirstCardBrand() -> CardBrandModel?
    ...
    """, callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
        ...
        @discardableResult
        func handleCobrandedCards<M1: Cuckoo.Matchable>(pan: M1) -> Cuckoo.__DoNotUse<(String), Void> where M1.MatchedType == String {
            let matchers: [Cuckoo.ParameterMatcher<(String)>] = [wrap(matchable: pan) { $0 }]
            return cuckoo_manager.verify(
    """
    handleCobrandedCards(pan: String)
    ...
    """, callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
        ...
        @discardableResult
        func getFirstCardBrand() -> Cuckoo.__DoNotUse<(), CardBrandModel?> {
            let matchers: [Cuckoo.ParameterMatcher<Void>] = []
            return cuckoo_manager.verify(
    """
    getFirstCardBrand() -> CardBrandModel?
