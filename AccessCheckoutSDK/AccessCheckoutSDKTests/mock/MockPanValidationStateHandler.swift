...
@testable import AccessCheckoutSDK
 ...
 class MockPanValidationStateHandler: PanValidationStateHandler, Cuckoo.ProtocolMock {
    ...
    return cuckoo_manager.call(
    """
    handleCobrandedCardsUpdate(brands: [CardBrandModel])
    """,
            parameters: (brands),
            escapingParameters: (brands),
            superclassCall:
                
                Cuckoo.MockManager.crashOnProtocolSuperclassCall()
                ,
            defaultCall: __defaultImplStub!.handleCobrandedCardsUpdate(brands: brands))
    ...
    """, parameterMatchers: matchers))
        ...
        func handleCobrandedCardsUpdate<M1: Cuckoo.Matchable>(brands: M1) -> Cuckoo.ProtocolStubNoReturnFunction<([CardBrandModel])> where M1.MatchedType == [CardBrandModel] {
            let matchers: [Cuckoo.ParameterMatcher<([CardBrandModel])>] = [wrap(matchable: brands) { $0 }]
            return .init(stub: cuckoo_manager.createStub(for: MockPanValidationStateHandler.self, method:
    """
    handleCobrandedCardsUpdate(brands: [CardBrandModel])
    """, parameterMatchers: matchers))
    ...
    """, callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
        ...
        @discardableResult
        func handleCobrandedCardsUpdate<M1: Cuckoo.Matchable>(brands: M1) -> Cuckoo.__DoNotUse<([CardBrandModel]), Void> where M1.MatchedType == [CardBrandModel] {
            let matchers: [Cuckoo.ParameterMatcher<([CardBrandModel])>] = [wrap(matchable: brands) { $0 }]
            return cuckoo_manager.verify(
    """
    handleCobrandedCardsUpdate(brands: [CardBrandModel])
