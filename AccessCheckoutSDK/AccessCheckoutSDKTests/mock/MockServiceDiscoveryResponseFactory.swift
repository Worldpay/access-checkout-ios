...
import Foundation
 ...
 class MockServiceDiscoveryResponseFactory: ServiceDiscoveryResponseFactory, Cuckoo.ClassMock {
    ...
    private var __defaultImplStub: ServiceDiscoveryResponseFactory?
     ...
     override func create(request: URLRequest, completionHandler: @escaping (Result<ApiResponse, AccessCheckoutError>) -> Void)  {
        
    return cuckoo_manager.call(
    """
    create(request: URLRequest, completionHandler: @escaping (Result<ApiResponse, AccessCheckoutError>) -> Void)
    """,
     ...
     struct __StubbingProxy_ServiceDiscoveryResponseFactory: Cuckoo.StubbingProxy {
        ...
        func create<M1: Cuckoo.Matchable, M2: Cuckoo.Matchable>(request: M1, completionHandler: M2) -> Cuckoo.ClassStubNoReturnFunction<(URLRequest, (Result<ApiResponse, AccessCheckoutError>) -> Void)> where M1.MatchedType == URLRequest, M2.MatchedType == (Result<ApiResponse, AccessCheckoutError>) -> Void {
            let matchers: [Cuckoo.ParameterMatcher<(URLRequest, (Result<ApiResponse, AccessCheckoutError>) -> Void)>] = [wrap(matchable: request) { $0.0 }, wrap(matchable: completionHandler) { $0.1 }]
    ...
    """, parameterMatchers: matchers))
     ...
     struct __VerificationProxy_ServiceDiscoveryResponseFactory: Cuckoo.VerificationProxy {
        ...
        @discardableResult
        func create<M1: Cuckoo.Matchable, M2: Cuckoo.Matchable>(request: M1, completionHandler: M2) -> Cuckoo.__DoNotUse<(URLRequest, (Result<ApiResponse, AccessCheckoutError>) -> Void), Void> where M1.MatchedType == URLRequest, M2.MatchedType == (Result<ApiResponse, AccessCheckoutError>) -> Void {
            let matchers: [Cuckoo.ParameterMatcher<(URLRequest, (Result<ApiResponse, AccessCheckoutError>) -> Void)>] = [wrap(matchable: request) { $0.0 }, wrap(matchable: completionHandler) { $0.1 }]
