...
class MockServiceDiscoveryResponseFactory: ServiceDiscoveryResponseFactory, Cuckoo.ClassMock {
    ...
    struct __StubbingProxy_ServiceDiscoveryResponseFactory: Cuckoo.StubbingProxy {
        ...
        func create<M1: Cuckoo.Matchable, M2: Cuckoo.Matchable>(request: M1, completionHandler: M2)
            -> Cuckoo.ClassStubNoReturnFunction<(URLRequest, (ApiResponse?) -> Void)>
        where M1.MatchedType == URLRequest, M2.MatchedType == (ApiResponse?) -> Void {
            let matchers: [Cuckoo.ParameterMatcher<(URLRequest, (ApiResponse?) -> Void)>] = [
                wrap(matchable: request) { $0.0 }, wrap(matchable: completionHandler) { $0.1 },
            ]
            return .init(
                stub: cuckoo_manager.createStub(
                    for: MockServiceDiscoveryResponseFactory.self,
                    method:
                        """
                        create(request: URLRequest, completionHandler: @escaping (ApiResponse?) -> Void)
    ...
    struct __VerificationProxy_ServiceDiscoveryResponseFactory: Cuckoo.VerificationProxy {
        ...
        @discardableResult
        func create<M1: Cuckoo.Matchable, M2: Cuckoo.Matchable>(request: M1, completionHandler: M2)
            -> Cuckoo.__DoNotUse<(URLRequest, (ApiResponse?) -> Void), Void>
        where M1.MatchedType == URLRequest, M2.MatchedType == (ApiResponse?) -> Void {
            let matchers: [Cuckoo.ParameterMatcher<(URLRequest, (ApiResponse?) -> Void)>] = [
                wrap(matchable: request) { $0.0 }, wrap(matchable: completionHandler) { $0.1 },
            ]
