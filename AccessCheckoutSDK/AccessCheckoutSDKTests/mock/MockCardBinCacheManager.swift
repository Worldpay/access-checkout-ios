...
class MockCardBinCacheManager: CardBinCacheManager, Cuckoo.ClassMock {
    ...
    struct __StubbingProxy_CardBinCacheManager: Cuckoo.StubbingProxy {
        ...
        func getCacheKey<M1: Cuckoo.Matchable>(from cardNumber: M1)
            -> Cuckoo.ClassStubFunction<(String), String> where M1.MatchedType == String
        {
            let matchers: [Cuckoo.ParameterMatcher<(String)>] = [wrap(matchable: cardNumber) { $0 }]
            return .init(
                stub: cuckoo_manager.createStub(
                    for: MockCardBinCacheManager.self,
                    method:
                        """
                        getCacheKey(from: String) -> String
        ...
        func getCachedResponse<M1: Cuckoo.Matchable>(for key: M1)
            -> Cuckoo.ClassStubFunction<(String), CardBinResponse?> where M1.MatchedType == String
        {
        ...
        func putInCache<M1: Cuckoo.Matchable, M2: Cuckoo.Matchable>(key: M1, response: M2)
            -> Cuckoo.ClassStubNoReturnFunction<(String, CardBinResponse)>
        where M1.MatchedType == String, M2.MatchedType == CardBinResponse {
            let matchers: [Cuckoo.ParameterMatcher<(String, CardBinResponse)>] = [
                wrap(matchable: key) { $0.0 }, wrap(matchable: response) { $0.1 },
            ]
            return .init(
                stub: cuckoo_manager.createStub(
                    for: MockCardBinCacheManager.self,
    ...
    struct __VerificationProxy_CardBinCacheManager: Cuckoo.VerificationProxy {
        ...
        @discardableResult
        func getCacheKey<M1: Cuckoo.Matchable>(from cardNumber: M1)
            -> Cuckoo.__DoNotUse<(String), String> where M1.MatchedType == String
        {
            let matchers: [Cuckoo.ParameterMatcher<(String)>] = [wrap(matchable: cardNumber) { $0 }]
        ...
        @discardableResult
        func getCachedResponse<M1: Cuckoo.Matchable>(for key: M1)
            -> Cuckoo.__DoNotUse<(String), CardBinResponse?> where M1.MatchedType == String
        {
            let matchers: [Cuckoo.ParameterMatcher<(String)>] = [wrap(matchable: key) { $0 }]
            return cuckoo_manager.verify(
                """
        ...
        @discardableResult
        func putInCache<M1: Cuckoo.Matchable, M2: Cuckoo.Matchable>(key: M1, response: M2)
            -> Cuckoo.__DoNotUse<(String, CardBinResponse), Void>
        where M1.MatchedType == String, M2.MatchedType == CardBinResponse {
            let matchers: [Cuckoo.ParameterMatcher<(String, CardBinResponse)>] = [
                wrap(matchable: key) { $0.0 }, wrap(matchable: response) { $0.1 },
            ]
