...
class ServiceDiscoveryProviderTests: XCTestCase {
    ...
    func testShouldDiscoverCardBinAndSessionsEndpoints() {
    ...
    private func setUpDiscoveryResponses(
        ...
        // mock access root and session discovery responses
        factoryMock.getStubbingProxy()
            .create(request: any(), completionHandler: any())
            .then {
                _, completionHandler in
                completionHandler(accessRootDiscoveryResult)
            }.then {
                _, completionHandler in
                completionHandler(sessionsDiscoveryResult)
            }
    }
